...
project_name = "darren-week6-object-detection"
...
with CustomVisionTrainingClient(TRAINING_ENDPOINT, training_credentials) as trainer, \
    ...
    # ── Bounding box regions (normalized: left, top, width, height) ───────────
    fork_image_regions = {
    ...
    base_image_location = os.path.join(os.path.dirname(__file__), "Images_Obj_Detection")
    logger.info("Uploading training images with bounding box regions...")

    tagged_images_with_regions = []

    for file_name, (x, y, w, h) in fork_image_regions.items():
        regions = [Region(tag_id=fork_tag.id, left=x, top=y, width=w, height=h)]
        with open(os.path.join(base_image_location, "fork", file_name + ".jpg"), "rb") as f:
            tagged_images_with_regions.append(
                ImageFileCreateEntry(name=file_name, contents=f.read(), regions=regions)
            )

    for file_name, (x, y, w, h) in scissors_image_regions.items():
        regions = [Region(tag_id=scissors_tag.id, left=x, top=y, width=w, height=h)]
        with open(os.path.join(base_image_location, "scissors", file_name + ".jpg"), "rb") as f:
            tagged_images_with_regions.append(
                ImageFileCreateEntry(name=file_name, contents=f.read(), regions=regions)
            )
    ...
    for prediction in results.predictions:
        ...
        print(f"  {prediction.tag_name}: {prediction.probability * 100:.2f}% "
              f"bbox.left={bb.left:.2f}, bbox.top={bb.top:.2f}, "
              f"bbox.width={bb.width:.2f}, bbox.height={bb.height:.2f}")