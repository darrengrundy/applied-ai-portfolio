...
The blocker has cleared. A fresh `gpt-5.2` (GlobalStandard, capacity 422) deployment succeeded in the `darren-guardrails-lab` Foundry project for Homework 6, confirmed via `az cognitiveservices account deployment show` (`provisioningState: Succeeded`, `deploymentState: Running`). No configuration change was made on the subscription side between the last failed attempt and this success — consistent with the corroborating Microsoft Q&A thread describing a platform-side issue rather than a per-subscription one. See [06-responsible-ai-guardrails](06-responsible-ai-guardrails/) for the completed exercise. Homework 4 and 5 have not yet been re-attempted against this cleared state.
